...
class Scenario(BaseScenario):
    ...
    def reset_world(self, world):
        ...
        for s in world.static_obs:
            ...
            s.state.p_pos = np.array([np.random.uniform(-500, 500), np.random.uniform(-500, 500)])
    ...
    def get_stream_value(self, agent):
        ...
        if agent.avoiding[0]:
            ...
            err += np.clip((1 / max(obs_dis[0], agent.size / 100) - 1 / (3 * agent.size / 100)), 0, None)
            #err += 1/c - 1/agent.C[0]
        ...
        if agent.avoiding[1]:
            ...
            err += np.clip((1 / max(obs_dis[1], agent.size / 100) - 1 / (3 * agent.size / 100)), 0, None)
