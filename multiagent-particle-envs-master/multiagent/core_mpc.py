    ...
    def integrate_state(self, p_force):
        for i,entity in enumerate(self.entities):
            ...
            if (p_force[i] is not None):
                ...
                entity.state.p_vel[0] = (p_force[i][1] * np.cos(entity.state.p_ang))
                entity.state.p_vel[1] = (p_force[i][1] * np.sin(entity.state.p_ang))
            ...
            # entity.state.p_pos += 100 * entity.state.p_vel * self.dt
            # entity.state.p_ang += entity.state.p_omg * self.dt
            if abs(entity.state.p_omg) > 1e-6:
                entity.state.p_pos[1] += 100 * speed / entity.state.p_omg * (
                            np.cos(entity.state.p_ang) - np.cos(entity.state.p_ang + self.dt * entity.state.p_omg))
                entity.state.p_pos[0] += 100 * speed / entity.state.p_omg * (
                            np.sin(entity.state.p_ang + self.dt * entity.state.p_omg) - np.sin(entity.state.p_ang))
                entity.state.p_ang += entity.state.p_omg * self.dt
            else:
                entity.state.p_pos[1] += 100 * speed * np.cos(entity.state.p_ang)
                entity.state.p_pos[0] += 100 * speed * np.sin(entity.state.p_ang)

            if entity.state.p_ang > 2*np.pi:
