...
from multiagent.core_mpc import World, Agent, Landmark, Static_obs, Dynamic_obs
...
class Scenario(BaseScenario):
    def make_world(self, arglist):
        ...
        world.ideal_topo_point = [[0,400,400,0],[0,0,400,400]]
    ...
    def reset_world(self, world):
        ...
        for e, agent in enumerate(world.agents):
            ...
            agent.state.p_ang = 0#math.atan2(world.goal[1] - world.start[1], world.goal[0] - world.start[0])
    ...
    def observation(self, agent, world):
        ...
        agt_x = [np.array([agent.state.p_pos[0]/100])-self.args.map_max_size/200 for agent in world.agents]
        agt_y = [np.array([agent.state.p_pos[1]/100])-self.args.map_max_size/200 for agent in world.agents]
        agt_yaw = [np.array([agent.state.p_ang]) for agent in world.agents]
        ...
        return np.concatenate(agt_dis + agt_ang + agt_x + agt_y + agt_yaw)
