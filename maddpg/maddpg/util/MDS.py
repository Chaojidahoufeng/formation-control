...
def f_Relative(groundtruth, P, N):
    ...
    I_Na = np.eye(N)
    # SVD
    M_h = copy.deepcopy(P.T)
    L = I_Na - np.ones([N, N]) * 1/N
    M = copy.deepcopy(groundtruth.T)
    ...
    P_final = np.reshape(P_final, (2, N), order='F')
    return P_final, gamma, t

def error_rel_g(p1, p2, n):
    p2_rel,_,_ = f_Relative(p1, p2, n)
    error2 = np.linalg.norm(p2_rel - p1)
    error_rel = np.sqrt(1/n * np.sum(error2 ** 2))

    return error_rel

def MDS_relative_error(groundtruth, shape_est, N):
    p2_rel, _, _ = f_Relative(groundtruth, shape_est, N)
    error2 = np.linalg.norm(p2_rel - groundtruth)
    error_rel = np.sqrt(1 / N * np.sum(error2 ** 2))
    return error_rel, p2_rel

def MDS_loc(observed_d, observed_theta, N):
    D = observed_d
    ...
    shape_est = V1[0:N,0:2]
    return np.copy(shape_est.T)
