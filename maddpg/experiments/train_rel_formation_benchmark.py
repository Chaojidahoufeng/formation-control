...
from maddpg.trainer.mpc import MPCTrainer
from maddpg.trainer.fuzzy_control import FuzzyTrainer
import tensorflow.contrib.layers as layers

def parse_args():
    parser = argparse.ArgumentParser("Reinforcement Learning experiments for multiagent environments")
    # Environment
    parser.add_argument("--scenario", type=str, default="rel_formation_only_mpc", help="name of the scenario script")
    ...
    parser.add_argument("--good-policy", type=str, default="mpc_model", help="policy for good agents")
    ...
    # Checkpointin1
    parser.add_argument("--exp-name", type=str, default='5-18-rel-formation-mpc', help="name of the experiment")
    ...
    parser.add_argument("--save-dir", type=str, default="model_maddpg_rel_formation_only_mpc.ckpt", help="directory in which training state and model should be saved")
    ...
    parser.add_argument("--form-rew-weight", type=float, default=0.01)
...
def make_env(scenario_name, arglist, benchmark=False):
    from multiagent.environment_mpc import MultiAgentEnv
...
def get_trainers(env, num_adversaries, his_shape_n, obs_shape_n, arglist):
    ...
    if arglist.network=="MLP":
        ...
        elif arglist.good_policy=="mpc_model":
            executor = MPCTrainer
            for i in range(env.n):
                trainers.append(executor(
                    "agent_%d" % i, i, arglist
                ))
        elif arglist.good_policy == "fuzzy_model":
            executor = FuzzyTrainer
            for i in range(env.n):
                trainers.append(executor(
                    "agent_%d" % i, i, arglist
                ))
        ...
        for episode in range(start_episode_num, arglist.num_episodes):
            ...
            while not (terminal or done):
                if arglist.network == "MLP":
                    ...
                    elif arglist.good_policy=="mpc_model" or arglist.good_policy=="fuzzy_model":
                        #print(obs_n)
                        action_n = [agent.action(obs) for agent, obs in zip(trainers, obs_n)] # [[ax0,ay0],[ax1,ay1]]
                    # environment step
                    if arglist.good_policy=="mpc_model" or arglist.good_policy=="fuzzy_model":
